    ...
    if classify_metrics:
        ...
        with tab3:
            with st.container(border=True):
                ...
                ops_col1.metric(
                    "P50 Latency / Doc",
                    f"{ops_metrics_result.get('p50_latency', 0):.2f}s",
                )
                ops_col2.metric(
                    "P95 Latency / Doc",
                    f"{ops_metrics_result.get('p95_latency', 0):.2f}s",
                )
                ops_col3.metric(
                    "Estimated Cost / Doc",
        ...
        st.info("No metrics available. Please ensure there is data to process.")
